...
-------------------------------------------------------------------------------
  ...
  signal bias_int              : std_logic :='0';
	...
	-- signal Calib_error           : std_logic :='0';
...
-------------------------------------------------------------------------------
    ...
    type CALIB_ENTRY_TYP is record          -- for calibration sequencer
      ...
      calib_rw      : std_logic;          -- note: wait when calib_rw ='1' and calib_data = X"FFFF",
                                          -- in this case address - wait time in 0.1 seconds !debug: 30/05/2023 0.02 seconds
    ...
    constant linearity_cal_c : CALIB_SEQ_TYP := ((X"57",X"03E8",'0'),(X"4D",X"0018",'0'), -- Linearity
                                                 ...
                                                 (X"52",X"0005",'0'),(X"FA",X"FFFF",'1'), -- X"14" - 2Sec, X"32" - 5Sec
    ...
    constant leakage_cal_c : CALIB_SEQ_TYP := ((X"56",X"0200",'0'),(X"4D",X"0000",'0'), -- Offset current
                                               ...
                                               (X"52",X"0007",'0'),(X"03",X"FFFF",'1'), -- X"0A" - 1Sec, X"1E" - 3Sec !debug: 29/05/2023 from Doron calc
                                               (X"52",X"0000",'0'),(X"53",X"1000",'0'),
                                               (X"58",X"0000",'1'));
    ...
    signal calib_wait_cnt : natural range 0 to 2**28 - 1; -- const delay
    ...
    attribute DONT_TOUCH of calib_ready, calib_monitor_reg, calib_monitor_running, READ_OUT_ready, calib_busy,
    ...
    attribute mark_debug of calib_ready, calib_monitor_reg, calib_monitor_running, READ_OUT_ready, calib_busy,
       calib_analize_cnt, calib_exec_cnt, calib_seq_cnt, calib_monitor_min_delay, aclk_cnt, EXE, configuring_sm_calib : signal is "TRUE";
...
-----------------------------------------------------------------------------
          ...
          when load_params =>
            ...
            if (calib_rw = '1') then -- RD
              if (calib_data = X"FFFF") then -- special case - wait in place read
                ...
                -- synthesis translate_off
                calib_wait_cnt <= 1e3 * conv_integer(calib_address);  -- for simulation only: 1mS
          ...
          when wait_for_ans =>
            ...
            if (time_out_timer >= x"1ffffff") then  -- !debug: 05/04/2021 x"3ffffff" -> x"1ffffff"
              ...
              -- if (calib = "001" and DATA2R(0) = '1') or (calib = "010" and DATA2R(1)='1') or (calib ="011" and DATA2R(2)='1') or (calib ="100" and DATA2R(3)='1') then
              --   configuring_sm_calib <= Wait_for_end;
              --   Calib_error          <= '0';
              -- elsif (calib = "001" and DATA2R(4) = '1') or (calib = "010" and DATA2R(5)='1') or (calib ="011" and DATA2R(6)='1') or (calib ="100" and DATA2R(7)='1') then
              --   configuring_sm_calib <= Wait_for_end;
              --   Calib_error          <= '1';
              -- elsif (calib = "001" and DATA2R(0) = '0') or (calib = "010" and DATA2R(1)='0') or (calib ="011" and DATA2R(2)='0') or (calib ="100" and DATA2R(3)='0') then
              --   configuring_sm_calib <= get_params;
              --   Calib_error          <= '0';
              -- end if;
              configuring_sm_calib <= Wait_for_end;  -- !debug: 11/05/2021 check DATA2R, when READ_OUT_ready = '1'
            end if; 
...
RAM_rd_en_o <= RAM_rd_en_int(1);        -- 29/06/2021
    ...
    if (RESET_N = '0') then
      ...
      if (calib_monitor_running and READ_OUT_ready = '1') then -- 08/11/2021 now calib_monitor_reg updates only in calibration time
        ...
        -- end if;
        calib_monitor_reg <= DATA2R; -- !debug: 31/05/2023 check, if preserved correct values for non-active calibrations
      end if;

      -- if (configuring_sm_regs = load_params and Instruction_int(7 downto 0) = X"0E") then -- 23/06/2021 POWER_MONITOR from sw
      if (do_calib = '1') then
      ...
      -- end if;
      do_power_check <= FALSE;
      if (configuring_sm_power = Wait_for_end and READ_OUT_ready = '1') then -- 23/06/2021
        power_monitor_reg <= DATA2R(11 downto 0);
      end if;

      if (calib_seq_cnt = 6 and READ_OUT_ready = '1') then -- 05/01/2022
  ...
  -- !debug: 27/06/2021 pulse   init_seq_end <= (init_seq_end_shr(1) or init_seq_end_shr(1)) and not init_seq_end_shr(3); -- 23/06/2021
