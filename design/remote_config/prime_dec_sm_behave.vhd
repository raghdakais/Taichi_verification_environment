...
begin
	if reset ='0' then
		...
		when delay2 =>      delay_counter <= delay_counter +'1';
			          ...
			          -- !debug: 19/06/2024 delays corrected for short simulation
		                    case command_aux is
		                    when "0000" =>  if delay_counter(5)='1'                 then end_sm<='1';sm2<= stop     ;delay_counter<=(others=>'0'); end if; --RDSR                --1 us
		                    when "0001" =>  if delay_counter(18)='1'                then end_sm<='1';sm2<= stop     ;delay_counter<=(others=>'0'); end if; --WRSR                --8 ms
		                    when "0010" =>  if delay_counter(13)='1'                then end_sm<='1';sm2<= stop     ;delay_counter<=(others=>'0'); end if; --4 bytes write       --256 us
		                    when "0011" =>  if delay_counter(6)='1'                 then end_sm<='1';sm2<= stop     ;delay_counter<=(others=>'0'); end if; --4 bytes read        --2 us
		                    when "0100" =>  if delay_counter(16)='1'                then end_sm<='0';sm2<= ongoing3 ;delay_counter<=(others=>'0'); end if; --write pp + read+crc --6.144 ms
		                    when "0101" =>  if delay_counter(27 downto 24)=x"2"     then end_sm<='1';sm2<= stop     ;delay_counter<=(others=>'0'); end if; --sector_erase        --1.04 sec
		                    when "0110" =>  if delay_counter(31 downto 20)=x"d97"   then end_sm<='1';sm2<= stop     ;delay_counter<=(others=>'0'); end if; --bulk_erase          --114 sec
		                    when others =>  if delay_counter(5)='1'                 then end_sm<='1';sm2<= stop     ;delay_counter<=(others=>'0'); end if; --RDSR                --1 us
		                    end case;
