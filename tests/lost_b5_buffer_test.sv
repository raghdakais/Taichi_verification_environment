// UVM test base for taichi_tmb environment

//===============================================================================
class lost_b5_buffer_test extends taichi_tmb_test_base;
//===============================================================================
    // Testbench configuration and stimulus generation
       `uvm_component_utils(lost_b5_buffer_test)  // Register with the factory
  
rand int i = 0;
 bit [31:0] wr_data ;
        // This is standard code for all components
    function new (string name = "lost_b5_buffer_test", uvm_component parent = null);
      super.new (name, parent);
    endfunction


//===============================================================================
    function void build_phase(uvm_phase phase);
//===============================================================================
        super.build_phase(phase);
    endfunction

    
//===============================================================================
    task run_phase (uvm_phase phase);
//===============================================================================
    	phase.raise_objection (this);
         	super.run_phase(phase);

        active_Mu_functional(0);
        active_synth_data(1);
   
    #1.5us;
    ////  0xB00X6790	Module Data Out Type selection
    send_operation_transaction(TXRX_WRITE, 'h406790, 'h0); 
    ////  0xB00X6750	Miscellaneous
    send_operation_transaction(TXRX_WRITE, 'h406750, 'h00004); 
    ////  0xB00X6790	Module Data Out Type selection
    send_operation_transaction(TXRX_READ, 'h064D0, 'h0); 

send_diagnostical_transaction(TXRX_WRITE, 'h064D0, 'hFFFF_FFFF); 

   ///     this.m_taichi_tmb_env.m_sync_txrx_agent.drv.allow_lost_B5 = 1;


    //------------------------------------------------------
    // Sending VALID HEADER WRITES
    //------------------------------------------------------
       send_sync_burst(4);

    //------------------------------------------------------
    // Sending VALID BUFFER READS
    //------------------------------------------------------

fork
    begin
      send_buffer_read_burst(1);
      this.m_taichi_tmb_env.m_buffer_tx_agent.drv.allow_lost_B5 = 1;
      send_buffer_read_burst(3);
      this.m_taichi_tmb_env.m_buffer_tx_agent.drv.allow_lost_B5 = 0;
      send_buffer_read_burst(4);

    end
    begin
        send_sync_burst(1);
        send_valid_sync_packet (0); 
        #60us;
        this.m_taichi_tmb_env.m_sync_txrx_agent.drv.allow_lost_B5 = 1;
        send_valid_sync_packet (1); 
        #80us;
        send_sync_burst(2);
        this.m_taichi_tmb_env.m_sync_txrx_agent.drv.allow_lost_B5 = 0;
        send_sync_burst(2);

    end
join


        #100us;

    	phase.drop_objection (this);
   endtask

endclass
